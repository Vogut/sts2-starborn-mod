...
/// <summary>
/// 实现此接口的 AbstractModel（遗物、卡牌、能力等）可在印记属性切换时收到通知。
/// 在 CombatState 的监听者列表中自动参与分发（与原版 Hook 机制一致）。
/// </summary>
public interface ISealAttributeChangedHook
{
    /// <summary>
    ...
    Task OnSealAttributeChanged(PlayerChoiceContext ctx, SealMarkPower mark, SealAttribute from, SealAttribute to);
}
